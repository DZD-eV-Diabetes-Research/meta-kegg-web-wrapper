...
def test_single_input_transcripts_run():
    res = req(
        "/api/pipeline",
        method="post",
    )
    ...
    res = req(
        ...
        method="patch",
        b={
            "global_params": {"sheet_name_genes": "transcript_metrics"},
            "method_specific_params": {},
        },
    )
    dict_must_contain(
        res["pipeline_params"]["global_params"],
        required_keys_and_val={"sheet_name_genes": "transcript_metrics"},
        exception_dict_identifier="PATCH-'/api/pipeline/{pipeline_ticket_id}'-response",
    )

    test_upload_file_single_input_gene_path = Path(
        PurePath(
            Path(__file__).parent, "provisioning_data/single_input_transcripts.xlsx"
        )
    )
    with open(test_upload_file_single_input_gene_path, "rb") as input_file:
        param_name = "input_file_path"
        res = req(
            f"/api/pipeline/{pipeline_ticket_id}/file/upload/{param_name}",
            method="post",
            form_file=("single_input_transcripts.xlsx", input_file),
        )
    dict_must_contain(
        res,
        required_keys_and_val={
    ...
    res = req(
        f"/api/pipeline/{pipeline_ticket_id}/run/single_input_transcripts",
        method="post",
    )
    dict_must_contain(
        res,
